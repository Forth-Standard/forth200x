...
\item \textbf{Standard Character Set}
	...
	\item Since a ``\emph{char}'' can store small positive numbers
		...
		unsigned integer data type, \word{C!} must store the $n$
...
\item \textbf{Manipulation on the stack}
	...
	\begin{quote}\ttfamily
		\word{toR}	\word{?DUP}	\word{DROP}	\word{DUP}
...
``\emph{c-addr} $u$'' representation of strings on the stack. The use
...
appear on the stack, it is preferable to use the ``\emph{c-addr}
$u$'' representation.
...
Per \xref{usage:number}{3.2.1.1 Internal number representation} and
...
There is historical precedent for limiting the range of $u$ to that
of $+n$, which is permissible when the cell size is greater than 16
bits.
...
The restrictions in section \xref{usage:returnstack}{3.2.3.3 Return stack}
...
\word{ENVIRONMENT?}'s database so that subsequent queries about it
...
\begin{quote}\ttfamily
	\word{:} LOOK \quad
		NCELLS 0 \word{DO}
			CELL-DATA \word{I} \word{CELLS} \word{+} \word[tools]{?}
		\word{LOOP}
	\word{;}
\end{quote}
...
Implementation note: Since the minimum value of $n$ is 16, the
absolute minimum size of the pictured numeric output string is 34
characters. But if your implementation has a larger $n$, you must
